...
serve(async (req) => {
  ...
  try {
    ...
    console.log("📦 Payload:", JSON.stringify(transactionPayload));
    ...
    if (!response.ok) {
      const beehiveMsg = data?.message || data?.error || JSON.stringify(data).substring(0, 300);
      console.error("❌ Beehive transaction failed:", response.status, beehiveMsg);
      // Return 200 so the frontend can read the actual error from the body
      return new Response(
        JSON.stringify({ ok: false, error: beehiveMsg, details: data, httpStatus: response.status }),
